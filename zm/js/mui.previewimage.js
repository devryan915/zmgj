(function($, window) {
	...
	var del = '<a href="#delete" class="mui-icon mui-icon-trash mui-pull-right"></a>';
	var delhtml = '<div id="delete" class="mui-popover mui-popover-action mui-popover-bottom"><ul class="mui-table-view"><li class="mui-table-view-cell mui-popover-header">要删除这张照片吗？</li><li class="mui-table-view-cell"><a href="#" data-tag="delete" style="color: #FF3B30;">删除</a></li></ul><ul class="mui-table-view"><li class="mui-table-view-cell"><a href="#delete"><b>取消</b></a></li></ul></div>';
	...
	proto.init = function() {
		...
		if(!el) {
			if(options.deleteCall) {
				options.header = del + options.header
				var sheet = document.createElement("div");
				sheet.innerHTML = delhtml;
	...
	proto.initEvent = function() {
		...
		$(document.body).on('tap', '[data-preview-src]', function() {
			self.options.openCall && self.options.openCall();
			self.open(this);
			return false;
		});
		var laterClose = null;
		var laterCloseEvent = function() {
			!laterClose && (laterClose = $.later(function() {
				self.options.closeCall && self.options.closeCall();
		...
		this.element.addEventListener('slide', function(e) {
			...
			cimgIndex = slideNumber;
		});
		self.options.deleteCall && $('#delete').on('tap', '.mui-popover-action li>a', function() {
			var a = this;
			if(a.dataset.tag == 'delete') {

				mui('#delete').popover('toggle');
				self.currentGroup.splice(self.lastIndex, 1)
				var cZoomerEl = self.element.querySelector('.mui-zoom-wrapper:nth-child(' + (self.lastIndex + 1) + ')');
				cZoomerEl.parentNode.removeChild(cZoomerEl);
				self.options.deleteCall && self.options.deleteCall(self.lastIndex);
				if(self.currentGroup.length == 0) {
					//关闭预览 
					self.close();
					return;
				}
				if(self.lastIndex == self.currentGroup.length) {

					self.refresh(self.lastIndex - 1, self.currentGroup)
				} else {
					if(self.options.zoom) {
						var lastZoomerEl = self.element.querySelector('.mui-zoom-wrapper:nth-child(' + (self.lastIndex + 1) + ')');
						if(lastZoomerEl) {
							lastZoomerEl.classList.add('mui-active')
							$(lastZoomerEl).zoom().setZoom(1);
						}
					}
					self._loadItem(self.lastIndex);
				}
				self.indicator && (self.indicator.innerText = (self.lastIndex + 1) + '/' + self.currentGroup.length);
	...
	proto.addImages = function(group, index) {
		...
		if(group) {
			if(group === defaultGroupName) {
				imgs = document.querySelectorAll("[data-preview-src]:not([data-preview-group])");
			} else {
				imgs = document.querySelectorAll("[data-preview-src][data-preview-group='" + group + "']");
			}
		} else {
			imgs = document.querySelectorAll("[data-preview-src]");
		}
		if(imgs.length) {
	...
	proto.close = function(index, group) {
		...
		var imgEl = itemEl && itemEl.querySelector('img');
		if(imgEl) {
			imgEl.classList.add($.className('transitioning'));
		...
		$(this.element).slider() && $(this.element).slider().destroy();
		//		this.empty();
	};
