define(['app', 'mui', 'vue', 'mui.pullToRefresh', 'mui.pullToRefresh.material', 'load'], function(app, mui, vue, mp, mpm, ld) {
	...
	headImage.src = 'http://chorse-sports.ufile.ucloud.com.cn/sys/indexlist/listmem_banner.png?v=1';
	var page;
	var vm = new vue({
		el: 'body', //绑定范围
		data: {
			...
			/*loadData: function(up) {
				...
				if(hasMore || (!hasMore && !up)) {
					...
					app.post(self.urlStr, self.reqPar, function(data) {
						...
						function() {
							up ? (self.ptr.endPullUpToRefresh(!hasMore) || console.log('上拉发生异常')) : (self.ptr.endPullDownToRefresh() || console.log('下拉发生异常'));
							//							app.closeWaiting();
						});
				} else {
		...
		ready: function() {
			...
			mui.ready(function() {
				...
				//				self.ptr.pullUpLoading();
				mui('.mui-scroll').on('tap', '.mem-star', function() {
					self.detail(this);
				});
				mui('.mem-main').on('tap', '.mem-item', function() {
					self.detail(this);
				});
