define(['app', 'mui', 'vue'], function(app, mui, vue) {
	...
	var vm = new vue({
			...
			load: function() {
				...
				app.post('api/News/GetNewsPage', self.par, function(data) {
					if(self.lower == 0) {
						self.up = data.UID;
						self.list = [];
						app.Cache.setObject(app.Cache.key.cclubData.toKeyName(), data.Items);
					}
					self.lower = data.LID;
		...
		created: function() {
			var self = this;
			var data = app.Cache.getArray(app.Cache.key.cclubData.toKeyName());
			self.list = data || [];
			mui.init({
				pullRefresh: {
					container: '#pr',
		...
		ready: function() {
			mui.plusReady(function() {
				mui('#news').on('tap', 'li', function() {
					var el = this;
					var w = plus.webview.getWebviewById('wclub_detail');
					if(w) {
						mui.fire(w, 'reload', {
							ref: {
								id: el.dataset.id,
