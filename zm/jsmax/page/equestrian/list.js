define(['app', 'mui', 'vue'], function(app, mui, vue) {
	...
	var vm = new vue({
			...
			load: function() {
				...
				app.post('api/News/GetNewsPage', self.par, function(data) {
					if(self.par.isUp == 0 && self.lower == 0) {
						self.up = data.UID;
						app.Cache.setObject(app.Cache.key.cequestrianData.toKeyName(), data.Items);
						self.list = [];
					}
					self.lower = data.LID;
		...
		created: function() {
			var self = this;
			var data = app.Cache.getArray(app.Cache.key.cequestrianData.toKeyName());
			self.list = data || [];
			mui.init({
				pullRefresh: {
					container: '#pr',
		...
		ready: function() {
			mui.plusReady(function() {
				mui('#equestrian').on('tap', 'li', function() {
					var el = this;
					var w = plus.webview.getWebviewById('wequestrian_detail');
					if(w) {
						mui.fire(w, 'reload', {
							ref: {
								id: el.dataset.id,
						...
						w = mui.preload({
							url: '../equestrian/detail.html',
							id: 'wequestrian_detail',
							styles: {
								popGesture: 'hide'
							},
							extras: {
